...
if config.cellranger.rerun_debug in falsy:
    config.cellranger.rerun_debug = None

# for legibility when creating sbatch command
slurm = config.cellranger.slurm

sbatch_file = require_path(path=project_dir / 'src' / 'sbatch-wrapper.sh', label='sbatch wrapper script', kind='file', create=False)
transcriptome_dir = require_path(config.cellranger.transcriptome, label='cellrange transcriptome', kind='dir', create=False)
...
if not library_id_file.exists():
    dat = pd.read_csv('DATA/syn51753257/AMP-AD_DiverseCohorts_assay_multiome_metadata.csv')['libraryBatch'].unique()
    df = pd.DataFrame({
        'N': range(1, len(dat)+1),
...
if config.cellranger.rerun_debug is None:
    for libraryID in df['libraryID']:
        ...
        elif not test_done_file.exists() or config.cellranger.clobber is True:
            sample_N = int(df.loc[df['libraryID'] == libraryID, 'N'].iloc[0])
            ids_to_run.append(sample_N)
else:
    try:
        sample_N = int(df.loc[df['libraryID'] == config.cellranger.rerun_debug, 'N'].iloc[0])
        ids_to_run.append(sample_N)
    except IndexError as e:
        raise RuntimeError(f"ERROR: Tried to run explicitly with a single sample, {config.cellranger.rerun_debug}, which does not exist in {config.data_dir}/libraryIDs.txt") from e
